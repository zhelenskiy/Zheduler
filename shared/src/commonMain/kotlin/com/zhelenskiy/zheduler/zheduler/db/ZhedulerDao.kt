...
data class BooleanFieldCount(val value: Long, val count: Long)
...
interface ZhedulerDao {
    ...
    """)
    suspend fun filterSpaces(searchInName: Long, query: String, searchInPrefix: Long): List<SpaceEntity>
    ...
    suspend fun getTasksBySpace(spaceId: String): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE spaceId = :spaceId AND id != :id")
    suspend fun getTasksBySpaceExcept(spaceId: String, id: String): List<TaskEntity>
    ...
    """)
    suspend fun searchTasksForConnection(spaceId: String, id: String?, searchQuery: String): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE id = :id")
    suspend fun getTaskById(id: String): TaskEntity?

    @Query("SELECT * FROM tasks WHERE id IN (:ids)")
    suspend fun getTasksByIds(ids: Set<String>): List<TaskEntity>
    ...
    @Query("SELECT * FROM tasks WHERE isRecurring = 1 AND dueDate IS NOT NULL AND dueDate <= :dueDate")
    suspend fun getRecurringTasksDueBefore(dueDate: Long): List<TaskEntity>
    ...
    suspend fun getConnectionsForTask(sourceTaskId: String): List<TaskConnectionEntity>

    @Query("INSERT OR REPLACE INTO task_connections(sourceTaskId, targetTaskId, type) VALUES (:sourceTaskId, :targetTaskId, :type)")
    suspend fun insertConnection(sourceTaskId: String, targetTaskId: String, type: String)

    @Query("DELETE FROM task_connections WHERE sourceTaskId = :sourceTaskId AND targetTaskId = :targetTaskId AND type = :type")
    ...
    """)
    suspend fun getParentTasks(sourceTaskId: String): List<TaskEntity>
    ...
    """)
    suspend fun getSubtasks(sourceTaskId: String): List<TaskEntity>

    // ==================== Status change queries ====================

    @Query("SELECT * FROM status_changes WHERE taskId = :taskId ORDER BY timestamp ASC")
    suspend fun getStatusTimeline(taskId: String): List<StatusChangeEntity>

    @Query("SELECT * FROM status_changes WHERE taskId IN (:taskIds) ORDER BY taskId, timestamp ASC")
    suspend fun getStatusTimelinesForTasks(taskIds: Set<String>): List<StatusChangeEntity>

    @Insert
    suspend fun insertStatusChange(statusChange: StatusChangeEntity)

    @Query("""
        SELECT sc.* FROM status_changes sc
        INNER JOIN tasks t ON sc.taskId = t.id
        WHERE t.spaceId = :spaceId AND sc.timestamp >= :startMs AND sc.timestamp < :endMs
    """)
    suspend fun getStatusChangesBySpaceAndDateRange(spaceId: String, startMs: Long, endMs: Long): List<StatusChangeEntity>
    ...
    suspend fun getAllCustomViewModes(spaceId: String): List<CustomViewModeEntity>

    @Query("SELECT * FROM custom_view_modes WHERE spaceId = :spaceId AND id = :id")
    suspend fun getCustomViewModeById(spaceId: String, id: String): CustomViewModeEntity?

    @Query("INSERT OR REPLACE INTO custom_view_modes(id, spaceId, name, configJson) VALUES (:id, :spaceId, :name, :configJson)")
    ...
    @Query("SELECT isRecurring AS value, COUNT(*) AS count FROM tasks WHERE spaceId = :spaceId GROUP BY isRecurring")
    suspend fun countTasksByIsRecurring(spaceId: String): List<BooleanFieldCount>

    @Query("SELECT isBlocked AS value, COUNT(*) AS count FROM tasks WHERE spaceId = :spaceId GROUP BY isBlocked")
    suspend fun countTasksByIsBlocked(spaceId: String): List<BooleanFieldCount>

    @Query("SELECT autoUpdateStatusFromSubtasks AS value, COUNT(*) AS count FROM tasks WHERE spaceId = :spaceId GROUP BY autoUpdateStatusFromSubtasks")
    suspend fun countTasksByAutoUpdateStatus(spaceId: String): List<BooleanFieldCount>
    ...
    suspend fun getTasksByStatusValues(spaceId: String, statusValues: Set<String>): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE spaceId = :spaceId AND status LIKE :statusPrefix")
    suspend fun getTasksByStatusPrefix(spaceId: String, statusPrefix: String): List<TaskEntity>
    ...
    """)
    suspend fun getTasksByPriorityRange(spaceId: String, includeNull: Long, minPriority: Long?, maxPriority: Long?): List<TaskEntity>
    ...
    """)
    suspend fun getTasksByDueDateRange(spaceId: String, includeNull: Long, minDueDate: Long?, maxDueDate: Long?): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE spaceId = :spaceId AND isRecurring = :value")
    suspend fun getTasksByIsRecurring(spaceId: String, value: Long): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE spaceId = :spaceId AND autoUpdateStatusFromSubtasks = :value")
    suspend fun getTasksByAutoUpdateStatus(spaceId: String, value: Long): List<TaskEntity>

    @Query("SELECT t.* FROM tasks t WHERE t.spaceId = :spaceId AND EXISTS (SELECT 1 FROM task_connections WHERE sourceTaskId = t.id)")
    suspend fun getTasksWithConnections(spaceId: String): List<TaskEntity>

    @Query("SELECT t.* FROM tasks t WHERE t.spaceId = :spaceId AND NOT EXISTS (SELECT 1 FROM task_connections WHERE sourceTaskId = t.id)")
    suspend fun getTasksWithoutConnections(spaceId: String): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE spaceId = :spaceId AND notificationsJson != '[]'")
    suspend fun getTasksWithNotifications(spaceId: String): List<TaskEntity>

    @Query("SELECT * FROM tasks WHERE spaceId = :spaceId AND notificationsJson = '[]'")
    suspend fun getTasksWithoutNotifications(spaceId: String): List<TaskEntity>
    ...
    // ==================== Count tasks by status with filters ====================
    ...
    // ==================== Count tasks by priority range with filters ====================
    ...
    // ==================== Comprehensive filtered task query with group filter ====================
    ...
    // ==================== Count tasks with comprehensive filters ====================
    ...
    @Query("""
        ...
        WHERE t.spaceId = :spaceId
          AND tt.tag IN (:tags)
    """)
    suspend fun getTaskIdsByTags(spaceId: String, tags: Set<String>): List<String>

    // ==================== Saved filter queries ====================

    @Query("SELECT * FROM saved_filters WHERE spaceId = :spaceId")
    suspend fun getAllSavedFilters(spaceId: String): List<SavedFilterEntity>

    @Query("SELECT * FROM saved_filters WHERE spaceId = :spaceId AND id = :id")
    suspend fun getSavedFilterById(spaceId: String, id: String): SavedFilterEntity?

    @Query("INSERT OR REPLACE INTO saved_filters(id, spaceId, name, criteriaJson, viewModeId) VALUES (:id, :spaceId, :name, :criteriaJson, :viewModeId)")
    suspend fun insertOrUpdateSavedFilter(id: String, spaceId: String, name: String, criteriaJson: String, viewModeId: String?)
