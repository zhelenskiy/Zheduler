...
abstract class IsMissedRepositoryTest : AbstractRepositoryTest {
    ...
    fun `task with no due date is not missed`() = runTest {
        ...
        val task = repo.add(spaceId, title = "No due date task")!!

        val taskWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == task.id }!!
    ...
    fun `task with future due date is not missed`() = runTest {
        ...
        val task = repo.add(spaceId, title = "Future task", dueDate = futureDue)!!
    ...
    fun `task with past due date and Open status is missed`() = runTest {
        ...
        val task = repo.add(spaceId, title = "Overdue task", dueDate = pastDue, status = TaskStatus.Open)!!
    ...
    fun `task with past due date and Done status is not missed`() = runTest {
        val (repo, spaceId) = createRepositoryWithSpace()
        val pastDue = Clock.System.now() - 1.days
        val task = repo.add(spaceId, title = "Completed task", dueDate = pastDue, status = TaskStatus.Done)!!
    ...
    fun `task with past due date and Declined status is not missed`() = runTest {
        val (repo, spaceId) = createRepositoryWithSpace()
        val pastDue = Clock.System.now() - 1.days
        val task = repo.add(spaceId, title = "Declined task", dueDate = pastDue, status = TaskStatus.Declined("reason"))!!
    ...
    fun `blocker task inherits missed status from dependent with past due date`() = runTest {
        ...
        val dependent = repo.add(spaceId, title = "Dependent with deadline", dueDate = pastDue, status = TaskStatus.Open)!!
        
        // Create a blocker task with no due date, but dependent depends on it
        val blocker = repo.add(spaceId, title = "Blocker task")!!
        repo.addConnection(dependent.id, blocker.id, ConnectionType.DependsOn)

        val blockerWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blocker.id }!!
    ...
    fun `blocker task not missed when dependent has future due date`() = runTest {
        ...
        val dependent = repo.add(spaceId, title = "Dependent with deadline", dueDate = futureDue, status = TaskStatus.Open)!!
    ...
    fun `blocker task uses closest due date from multiple dependents`() = runTest {
        ...
        val dependent1 = repo.add(spaceId, title = "Dependent 1", dueDate = futureDue)!!
        val dependent2 = repo.add(spaceId, title = "Dependent 2", dueDate = pastDue)!!
    ...
    fun `task blocked by another task inherits missed status via Blocked status`() = runTest {
        ...
        val blockerTask = repo.add(spaceId, title = "Blocker")!!
        ...
        val blockedWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blockedTask.id }!!
        val blockerWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blockerTask.id }!!
    ...
    fun `completed blocker task is not missed even with past total due date`() = runTest {
        ...
        // Create a dependent task with past due date
        val dependent = repo.add(spaceId, title = "Dependent", dueDate = pastDue)!!
        
        // Create a blocker task that is already done
        val blocker = repo.add(spaceId, title = "Blocker", status = TaskStatus.Done)!!
        repo.addConnection(dependent.id, blocker.id, ConnectionType.DependsOn)
    ...
    fun `task with own future due date but past total due date from dependency is missed`() = runTest {
        ...
        val blocker = repo.add(spaceId, title = "Blocker", dueDate = futureDue)!!
        repo.addConnection(dependent.id, blocker.id, ConnectionType.DependsOn)
    ...
    fun `chain of dependencies propagates missed status`() = runTest {
        ...
        val taskA = repo.add(spaceId, title = "Task A", dueDate = pastDue)!!
        val taskB = repo.add(spaceId, title = "Task B")!!
        val taskC = repo.add(spaceId, title = "Task C")!!
        ...
        val tasksWithTotals = repo.getAllWithTotals(spaceId)
    ...
    fun `task with past due date and InProgress status is missed`() = runTest {
        ...
        val now = Clock.System.now()
        val pastDue = now - 1.days
        val task = repo.add(spaceId, title = "InProgress overdue task", dueDate = pastDue, status = TaskStatus.InProgress)!!
    ...
    fun `parent task inherits totalDueDate from subtask with earlier due date`() = runTest {
        ...
        val parent = repo.add(spaceId, title = "Parent task", dueDate = futureDue)!!
        
        // Create subtask with past due date
        val subtask = repo.add(spaceId, title = "Subtask", dueDate = pastDue)!!
        repo.addConnection(subtask.id, parent.id, ConnectionType.SubtaskOf)

        val parentWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == parent.id }!!
    ...
    fun `subtask with no due date but parent has past due date - subtask not missed`() = runTest {
        ...
        val parent = repo.add(spaceId, title = "Parent task", dueDate = pastDue)!!
        
        // Create subtask with no due date
        val subtask = repo.add(spaceId, title = "Subtask")!!
        repo.addConnection(subtask.id, parent.id, ConnectionType.SubtaskOf)

        val subtaskWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == subtask.id }!!
    ...
    fun `nested subtasks - grandchild inherits missed status via dependency chain`() = runTest {
        ...
        val grandparent = repo.add(spaceId, title = "Grandparent", dueDate = pastDue)!!
    ...
    fun `task becomes not missed after status changes to Done`() = runTest {
        ...
        // Initially missed
        var taskWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == task.id }!!
        assertTrue(taskWithTotals.isMissed(now))
        ...
        // No longer missed
        taskWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == task.id }!!
    ...
    fun `task becomes not missed after status changes to Declined`() = runTest {
        ...
        // Mark as Declined
        repo.update(repo.getById(task.id)!!.copy(status = TaskStatus.Declined("No longer needed")))
    ...
    fun `blocker task becomes not missed after completing - dependent still missed`() = runTest {
        ...
        var blockerWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blocker.id }!!
        var dependentWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == dependent.id }!!
        assertTrue(blockerWithTotals.isMissed(now))
        ...
        blockerWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blocker.id }!!
        dependentWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == dependent.id }!!
        assertFalse(blockerWithTotals.isMissed(now))
        assertTrue(dependentWithTotals.isMissed(now))
    }
    ...
    fun `task unblocked automatically still missed if past due`() = runTest {
        ...
        var taskWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blockedTask.id }!!
        assertTrue(taskWithTotals.isMissed(now))
        ...
        taskWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == blockedTask.id }!!
    ...
    fun `dependency inherits totalDueDate from dependent task and becomes missed`() = runTest {
        ...
        val dependency = repo.add(spaceId, title = "Dependency")!!

        // Dependency initially not missed (no due date, no connections)
        var dependencyWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == dependency.id }!!
        ...
        dependencyWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == dependency.id }!!
    ...
    fun `completing all subtasks makes parent Done and not missed`() = runTest {
        ...
        // Create subtasks
        val subtask1 = repo.add(spaceId, title = "Subtask 1")!!
        val subtask2 = repo.add(spaceId, title = "Subtask 2")!!
        ...
        var parentWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == parent.id }!!
        ...
        parentWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == parent.id }!!
    ...
    fun `isMissed changes based on time passing`() = runTest {
        ...
        val task = repo.add(spaceId, title = "Task due soon", dueDate = soonDue)!!
    ...
    fun `ParentOf connection does not propagate totalDueDate from child to parent`() = runTest {
        ...
        val child = repo.add(spaceId, title = "Child", dueDate = pastDue)!!
        ...
        val childWithTotals = repo.getAllWithTotals(spaceId).find { it.task.id == child.id }!!
    ...
    fun `SubtaskOf connection does not propagate totalDueDate from parent to child`() = runTest {
        ...
        // Create parent with past due date
        val parent = repo.add(spaceId, title = "Parent", dueDate = pastDue)!!
    ...
    fun `adding DependsOn connection updates totalDueDate for dependency`() = runTest {
        ...
        val taskWithDeadline = repo.add(spaceId, title = "Task with deadline", dueDate = pastDue)!!
        val dependency = repo.add(spaceId, title = "Dependency")!!
    ...
    fun `updating task due date updates totalDueDate for dependencies`() = runTest {
        ...
        val taskWithDeadline = repo.add(spaceId, title = "Task with deadline", dueDate = futureDue)!!
        val dependency = repo.add(spaceId, title = "Dependency")!!
        repo.addConnection(taskWithDeadline.id, dependency.id, ConnectionType.DependsOn)
        ...
        val currentTask = repo.getById(taskWithDeadline.id)!!

        // Update task's due date to past
        repo.update(currentTask.copy(dueDate = pastDue))
    ...
    fun `changing task status to Blocked propagates totalDueDate to new blockers`() = runTest {
        ...
        val task = repo.add(spaceId, title = "Task", dueDate = pastDue)!!
        ...
        repo.update(repo.getById(task.id)!!.copy(status = TaskStatus.Blocked(setOf(blocker.id))))
    ...
    fun `changing task status from Blocked removes totalDueDate propagation from old blockers`() = runTest {
        ...
        repo.update(repo.getById(blockedTask.id)!!.copy(status = TaskStatus.Open))
